...
#Importing data
survey <- read.csv("C:/Users/Hemanshi/Desktop/Langara/Term 3/CPSC - Visualization for DA/Final Exam/Marwaha_Hemanshi_RShiny/survey_cleaned.csv")
str(survey)
...
geo_location <- read.csv("C:/Users/Hemanshi/Desktop/Langara/Term 3/CPSC - Visualization for DA/Final Exam/Marwaha_Hemanshi_RShiny/countries.csv")
geo_location
...
countries<-unique(sort(survey$Country))
...
states <- unique(sort(survey$state))
...
server <- function(input, output) {
    ...
    output$map <- renderLeaflet({
        ...
        survey <- map_data_filtered()
        
        if(nrow(survey) == 0)
            return(  leaflet( options = leafletOptions(minZoom = 2, maxZoom = 5)) %>%
                         addProviderTiles("OpenStreetMap.Mapnik"))
        
        values <- unique(survey[, input$singleColumn])
        
        spread_data <-survey %>%
            group_by(Country, value = survey[1:nrow(survey), input$singleColumn]) %>%
            summarise(n = n())
        ...
        leaflet(survey = spread_data, options = leafletOptions(minZoom = 2, maxZoom = 5)) %>%
            addProviderTiles("OpenStreetMap.Mapnik") %>% 
